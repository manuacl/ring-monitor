...
- feat(appearance): optional background halo behind the rings (#170). Five new Appearance keys — `backgroundEnabled` (default `false`, so the widget stays fully transparent), `backgroundColor`, `backgroundOpacity`, `backgroundSpread` (0-100 % of the ring radius added around the rings) and `backgroundEdgeSoftness` (0-50 % of the halo's shorter side faded linearly to transparent) — render through the new `core/WidgetBackground.qml`: a stadium drawn by one `Shape`, shaped on the ring layout rather than the host (each host passes its `MainContent` as `rings`; `BackgroundStyle.ringBounds()` unions the square each `Ring` draws in its cell), so the caps follow the end rings and the halo stays on them when a Plasma applet is resized bigger than the strip. The soft edge is painted, not blurred — a `LinearGradient` across the body and a `RadialGradient` on each half-disc cap (`core/HaloCap.qml`), seams on whole pixels; a `MultiEffect` blur read as the plate shrinking. Geometry lives in the pure `core/BackgroundStyle.js`. On Plasma, `fullRepresentation` is now a wrapper `Item` holding the halo and `MainContent` as siblings — a `GridLayout` gives every child a cell — forwarding `contentBody.implicitWidth/Height` so the panel allocation stays ring-driven. The controls live in their own `core/BackgroundSettings.qml` (`AppearanceBody` was at the 500-line cap, which also cost two over-long comment blocks, now pointers to `docs/`), with `AppearanceBody` re-exposing the properties by alias so the `cfg_*` bridge and the standalone `_bridgeMap` carry them like every other key. In standalone the window grows around the rings to make room for a spread halo (`WindowPlacement.haloInsets`), borrowing from `windowMarginX/Y` on the anchored sides so the rings stay put on screen; a halo wider than the margin is cut by the screen edge instead of pushing the window off-screen.
