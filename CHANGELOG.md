...
- feat(appearance): adjustable soft edges on the background plate (#170). New `backgroundEdgeSoftness` key (0-33 % of the shorter side, default `0` = the crisp rectangle): a linear `Gradient` fades along one axis only, so all four edges are feathered by a `QtQuick.Effects` `MultiEffect` blur instead. The plate is inset by `BackgroundStyle.featherPixels()` and the effect anchored to it with `autoPaddingEnabled`, so the fade lands inside the widget instead of being clipped at its edge; the effect is always instantiated and switched by `blurEnabled` (never by `visible` — `MultiEffect` owns its source's visibility, so hiding it would blank the plate). `featherPixels` clamps to the effect's own 64 px `blurMax` ceiling and returns 0 for the pre-layout `width`/`height == 0` pass.
- feat(appearance): the background plate is a halo shaped on the rings, not the window (#170). Each host passes its `MainContent` to `WidgetBackground.rings`; the new pure `BackgroundStyle.ringBounds()` unions the centred `min(w, h)` square each `Ring` delegate draws in its cell into a stadium whose caps carry the ring radius, so the plate follows the end rings and stays on them when a Plasma applet is resized bigger than the strip. The soft-edge inset shrinks the caps by the same amount, keeping them concentric.
