...
| `buildThermalCatalog(zones)` | **Fallback** catalog from `/sys/class/thermal` zones, used only when hwmon exposes no temperature input at all (some ARM boards / VMs register temps only with the thermal framework; on x86 the zones mirror hwmon chips, so mixing both would double the picker list — `HwmonTempSensors.enumerate()` calls it only on an empty hwmon catalog). Same grammar: `<type>/temp`, `<type>@<device>/temp` on type collision (the zone number is registration-order, never persisted). Zones have no label file — the type is the name, left bare (no redundant `acpitz (acpitz)` stem suffix). |
