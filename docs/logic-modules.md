...
| `resolveDiskRingIds(manualIds, removableMounts, optOutIds, defaultIds, maxCount)` | The final ordered disk-ring set rendered by `MainContent`: the manual selection (`manualIds`, already ordered) first, then each currently-mounted removable filesystem (`removableMounts = [{id,…}]`) not already selected and not in `optOutIds` (the **auto-show**), deduped; falls back to `defaultIds` when the union is empty (`[]` = aggregate ring on Plasma, the `$HOME` FS on standalone), capped at `maxCount`. Driving the removable members off the live mount set — not ksysguard — is what self-heals the ring on unplug ([#58](https://github.com/manuacl/ring-monitor/issues/58)). `optOutIds` is `[]` until Phase 3 adds its config key. |
